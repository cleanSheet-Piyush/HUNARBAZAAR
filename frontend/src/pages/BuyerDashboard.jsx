...
const ZeroCommissionBadge = ({ style }) => (
  <div
    ...
    title="HUNARBAZAAR takes zero commission — the full amount you pay goes to the artisan."
  >
...
const BuyerDashboard = () => {
  ...
  const loadProducts = async (query = "") => {
    ...
    try {
      ...
      const localProducts = JSON.parse(localStorage.getItem("hunarbazaar_custom_products") || "[]");
  ...
  useEffect(() => {
    ...
    window.addEventListener("hunarbazaar_product_added", handleCustomAdd);
    return () => window.removeEventListener("hunarbazaar_product_added", handleCustomAdd);
  }, []);
  ...
  return (
    <div className="voice-panel-page">
      ...
      <div className="regalia-hero-banner">
        <h1>HUNARBAZAAR BOUTIQUE MARKETPLACE</h1>
        ...
        <div className="regalia-layout">
          ...
          <Sidebar
            ...
            onSuggestionSelect={(term) => {
              // Try to find an exact or close matching product and open its detail
              const match = products.find(
                (p) => p.title && p.title.toLowerCase().includes(term.toLowerCase())
              );
              if (match) {
